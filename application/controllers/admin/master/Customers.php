...
class Customers extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        define('THIS_URL', base_url('admin/master/customers'));
    ...
    public function index()
    {
        ...
        // get list
        $dt_rows = $this->customers_model->get_list($limit, $offset, $filters, $sort, $dir);
        ...
        $this->set_title(lang('customers title list'));
        ...
        // load views
        $data['content'] = $this->load->view('admin/master/customers/list', $content_data, true);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // save
            $saved = $this->customers_model->do_save($this->input->post(), $active_user['id']);
        ...
        // setup page header data
        $this->set_title(lang('customers title add'));
        ...
        // load views
        $data['content'] = $this->load->view('admin/master/customers/form', $content_data, true);
        $this->load->view($this->template, $data);
    }
    ...
    public function edit($id = null)
    {
        ...
        // get the data
        $dt = $this->customers_model->as_array()->get($id);
        ...
        // setup page header data
        $this->set_title(lang('customers title edit'));
    ...
    public function delete()
    {
        if ($_POST && isset($_POST['id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!is_null($id) or !is_numeric($id)) {
                ...
                $dt = $this->customers_model->as_array()->get($id);
                if ($dt) {
                    $deleted = $this->customers_model->do_delete($dt, $active_user['id']);
                    if ($deleted) {
    ...
    public function export()
    {
        ...
        // get list
        $dt_rows = $this->customers_model->get_list(0, 0, $filters, $sort, $dir);
        ...
        if ($dt_rows['total'] > 0) {
            ...
            // export the file
            array_to_csv($dt_rows['results'], lang('customers list title'));
        } else {
            // nothing to export
            $this->session->set_flashdata('error', lang('global error no_results'));
            redirect($this->_redirect_url);
    ...
    private function form_validators($dt = array(), $new_data = true)
    {
        ...
        if ($new_data) {
            $this->form_validation->set_rules('nama', lang('customers input nama'), 'required|trim|min_length[3]|max_length[32]|callback__check_nama[]');
        } else {
            $this->form_validation->set_rules('nama', lang('customers input nama'), 'required|trim|min_length[3]|max_length[32]|callback__check_nama[' . $dt['nama'] . ']');
        }
        $this->form_validation->set_rules('alamat', lang('customers input alamat'), 'required|trim|max_length[255]');
        $this->form_validation->set_rules('status', lang('customers input status'), 'required|trim');
        $this->form_validation->set_rules('keterangan', lang('customers input keterangan'), 'required|trim');
    }

    /**
     * Enum pada field customers.status
     */
    ...
    {
        if (trim($nama) != trim($current) && $this->customers_model->nama_exists($nama)) {
            $this->form_validation->set_message('_check_nama', sprintf(lang('customers error nama_exists'), $nama));
            return false;
        } else {
            return $nama;
        }
