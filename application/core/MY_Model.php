 ...
 *
 * Beberapa penyesuaian sudah dilakukan oleh Khairul Anwar <https://irul.opensipkd.com>
...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        /** irul @ 20161016 */
        array_unshift($this->clean_datas, 'remove_nonfields');
    ...
    /* --------------------------------------------------------------
     * Penambahan/penyesuaian oleh irul
     * ------------------------------------------------------------ */

    /**
     * Skip triger everywhere
     */
    protected $skip_trigger = false;

    /**
     * Return the fields of table
     */
    ...
    /**
     * Tell the class to skip the trigger
     */
    public function skip_trigger()
    {
        $this->skip_trigger = true;
        return $this;
    }

    /**
     * Get the skip trigger status
     */
    public function get_skip_trigger()
    {
        return $this->skip_trigger;
    }
    ...
    {
        if (empty($this->_fields) && $this->_table != 'mies') {
            $query  = $this->db->query('SELECT * FROM ' . $this->_table . ' WHERE false');
            $fields = array();
            foreach ($query->list_fields() as $field) {
                $fields[] = $field;
            }
            $this->_fields = $fields;
            $_num_fields   = count($fields);
        }
    }

    /**
     * remove non exist column in fields
     */
    protected function remove_nonfields($data)
    {
        ...
        // FIXME : blm test untuk handle type object
    ...
    public function insert($data, $skip_validation = false)
    {
        ...
        if ($data !== false) {
            ...
            /** irul @ 20161016 */
            $data = $this->trigger('clean_datas', $data);
